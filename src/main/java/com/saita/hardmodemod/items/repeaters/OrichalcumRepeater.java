...
public class OrichalcumRepeater extends ShootableItem {
   public OrichalcumRepeater(Item.Properties builder) {
      super(builder);
         ...
         if (!itemstack.isEmpty() || flag) {
            ...
            if (!((double)f < 0.1D)) {
               ...
               if (!worldIn.isRemote) {
                  ...
                  abstractarrowentity.setDamage(4.0D); // ! Repeater Arrow Base Damage !
                  if (f == 1.0F) {
   ...
   public static float getArrowVelocity(int charge) {
      float f = (float)charge / 8.0F; // <--- ! Repeater Shooting Speed, the lower, the faster. !
      f = (f * f + f * 2.0F) / 3.0F;
      if (f > 1.0F) {
         f = 1.0F;
      }
   ...
   // ! Repeater durability !
    ...
    public boolean getIsRepairable(ItemStack toRepair, ItemStack repair) {
       ...
       return repair.getItem() == ItemInit.ORICHALCUM_INGOT.get() || repair.getItem() == ItemInit.ORICHALCUM_REPEATER.get() ? true : super.getIsRepairable(toRepair, repair);
