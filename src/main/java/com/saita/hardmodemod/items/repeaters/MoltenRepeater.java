...
public class MoltenRepeater extends ShootableItem {
   public MoltenRepeater(Item.Properties builder) {
      super(builder);
         ...
         if (!itemstack.isEmpty() || flag) {
            ...
            if (!((double)f < 0.1D)) {
               ...
               if (!worldIn.isRemote) {
                  ...
                  abstractarrowentity.setDamage(4.5D);// ! Repeater Arrow Base Damage !
                  abstractarrowentity.setFire(25); // ! Set arrow on fire !
   ...
   public static float getArrowVelocity(int charge) {
      float f = (float)charge / 10.0F; // <--- ! Repeater Shooting Speed, the lower, the faster. !
      f = (f * f + f * 2.0F) / 3.0F;
      if (f > 1.0F) {
         f = 1.0F;
      }
   ...
   // ! Repeater durability !
    ...
    public boolean getIsRepairable(ItemStack toRepair, ItemStack repair) {
       ...
       return repair.getItem() == ItemInit.MOLTEN_BAR.get() || repair.getItem() == ItemInit.MOLTEN_REPEATER.get() ? true : super.getIsRepairable(toRepair, repair);
